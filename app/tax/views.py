...
def add_tax():
    ...
    try:
        r = tax_mapper.get_obj_from_request(data, g.customer)
    ...
    response_object = jsonify({
        "data": tax_mapper.get_response_object(r.full_serialize()),
        "status" : 'success',
...
@tax.route("/<string:taxId>", methods = ["DELETE"])
@auth.login_required
def delete_tax(taxId):
    if not feeId:
        return common_views.bad_request(constants.view_constants.REQUEST_PARAMETERS_NOT_SUFFICIENT)
    fee_id = feeId
...
@tax.route("/<string:taxId>", methods = ["GET"])
@auth.login_required
def get_single_tax(taxId):
    if not taxId:
        return common_views.bad_request(constants.view_constants.REQUEST_PARAMETERS_NOT_SUFFICIENT)
    tax_id = taxId
    gp = Tax.query.get(taxId)
    if gp: 
        data = {        
            "name"     : gp._name,                   
